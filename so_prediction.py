...
print(np.mean(sample_coefs, axis=0))
...
MRI_Results.to_filename('out_nii_so.nii')


# get ROI with highest weights name
main_res = {}
results = {}
for i, roi in enumerate(atlas.labels):
	results[roi] = Weight_results[i]
	if Weight_results[i] < -0.3 or Weight_results[i] > 0.3:
		main_res[roi] = Weight_results[i]

# store results in dataframe
df_results = pd.DataFrame(main_res.items())
...
df_sign = pd.DataFrame(dic.items())
...
MRI_Results.to_filename('significant_nii_so.nii')


# extract atlas
MRI_Results = masker.inverse_transform(np.arange(100).reshape(1, 100))
MRI_Results.to_filename('atlas_schachter.nii')
...
result_confound = {}
for confound in counfounds:
	my_confound = df[confound].values
	...
	for i_subsample in range(100):
		...
		for train_index, test_index in sss.split(X_brain_, Y_):
			...
			sample_accs.append(acc)
			sample_coefs.append(clf.coef_[0, :])
	# Store the results for each confound
	result_confound[confound[0]] = [np.mean(sample_accs), np.std(sample_accs), np.mean(sample_coefs, axis=0)]
