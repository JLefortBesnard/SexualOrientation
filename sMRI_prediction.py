...
Weight_mean = np.mean(sample_coefs, axis=0)
Weight_std = np.std(sample_coefs, axis=0)
MRI_Results = masker.inverse_transform(Weight_mean.reshape(1, 100))
MRI_Results.to_filename('MRIcoef_deconfounded.nii')
MRI_Results = masker.inverse_transform(Weight_std.reshape(1, 100))
MRI_Results.to_filename('MRIcoef_std_deconfounded.nii')
...
MRI_Results.to_filename('significantROI_deconfounded.nii')
